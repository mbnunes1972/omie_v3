...
| 42 | `comissao_arq_pct`/`fidelidade_pct` sem limite → `Val_Liq` pode ir negativo → markup negativo → Δ a cobrar (B4) inverte sinal contra Δ custo → backend (`decisao_valida`, ancorado em Δ custo) recusa qualquer botão que a tela ofereça | `test_medicao_achado42_markup_negativo.py` — computado (não hipotético): comissão de 150% → `Markup=-1.0`; Δ custo +1000 → Δ a cobrar -1000 (fallback); `decisao_valida` só aceita absorver/cobrar pra Δ custo positivo, nunca manter/estornar | **SEM PROVA (medição, sem conserto)** 02/09 — candidatos: limitar comissão/fidelidade (mesmo padrão do `limite_desconto`), ou `decisao_valida` passar a usar Δ a cobrar também; nenhum decidido |
