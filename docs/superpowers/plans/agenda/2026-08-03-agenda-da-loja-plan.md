...
- Config da loja: seção `agenda` no `config_financeira_json` (ou bloco próprio):
  `produtividade_montagem_rs_dupla_dia=7000`, `duplas_disponiveis=2`, `sabado_util=false`,
  `feriados=[]`, `teto_dias_montagem=10`, `horizonte_capacidade_semanas=6`. Painel Config →
  campos editáveis (inclui lista de feriados).
...
- `mod_agenda.cargas_dia(...)`: entrega-no-período para todos os setores; Montagem distribuída
  (janela §6 da spec via `mod_calendario.espalhar`).
...
- `mod_agenda.capacidade(...)`: duplas necessárias/dia (⌈Σ carga/produtividade⌉), horizonte
  configurável.
- Painel com barras + linha de duplas disponíveis + estouros; rodapé de duplas no Calendário;
  linha de capacidade na Semana; dias-dupla no Mês.
