...
| **F2-20** · ACHADO-49 (Remover da etapa 12) + ACHADO-50 (pendente reportado como falha) + ACHADO-51 (NF-e da fábrica em duplicata) — a mesma família "a tela não conta o que o servidor fez" (ACHADOS 32/33/38/39/41) | **próximo** — os três achados do percurso do `v2026.09.04-beta1` em Homologação (04/09), tratados juntos por serem a mesma família; ACHADO-49: autoridade do Remover da etapa 12 não espelha o upload (herdou `executar_pe` do `else` pensado pro PE) + `removerDocCiclo` cala quando a credencial não vem (ACHADO-38 outra vez); ACHADO-50: nota `PROCESSANDO` além do timeout de 60s vira "Falha na emissão", quando é só pendência normal na fila da SEFAZ; ACHADO-51: nada impede carregar a mesma NF-e da fábrica duas vezes — DECIDIDO bloquear, por chave do XML, respeitando o ACHADO-30 (`_docs_vivos`) |
