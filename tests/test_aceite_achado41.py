...
def test_linha_em_sobra_nao_oferece_absorver(http_client_factory, seed, app_db):
    nome = "ACHADO41_sobra"
    ...
    assert set(linha["vereditos_validos"]) == {"receber", "adiar"}
    assert "absorver" not in linha["vereditos_validos"]

    # o backend continua recusando por trás — o campo é DERIVADO, não redundante por acaso
    try:
        mc.resolver_veredito_provisao(db, ot, oid, nome, "2.1.04.02", "absorver", ref="achado41:sobra:tenta")
        assert False, "deveria recusar"
    except ValueError:
        pass


def test_linha_em_falta_nao_oferece_receber_nem_encerrar(http_client_factory, seed, app_db):
    nome = "ACHADO41_falta"
    _projeto_pronto_para_etapa_21(app_db, seed, nome)
    ot, oid = _constituir_montagem(app_db, seed, nome, valor=1000.0)
    db = app_db.get_session()
    ...
    assert set(linha["vereditos_validos"]) == {"absorver", "adiar"}
    assert "receber" not in linha["vereditos_validos"]
    assert "encerrar" not in linha["vereditos_validos"]
