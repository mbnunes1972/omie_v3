...
contra o localhost (banco do `DATABASE_URL` do `.env`) linha a linha, por CHAVE NATURAL — nunca
por id, que diverge entre ambientes por construção. Existe pra não depender de alguém lembrar de
rodar a comparação a mão antes de promover a migration de dado pra Integração/Homologação.
...
def test_migration_de_dado_reproduz_centro_custo_e_conta_do_localhost():
    ...
    assert not divergencias, (
        f"Migration nao reproduz o localhost ({len(divergencias)} diferenca(s) por chave "
