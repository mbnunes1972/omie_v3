...
| **F2-9** · Parte B do `docs/db/TAREFA_PERCURSO_0109.md` (B1/B2, percurso de 01/09) | **em andamento** — 02/09, `cba0159`; B1 (ACHADO-35) **RESOLVIDO**: recusa virou confirmação, total do dia vem do razão (`efetivado_no_dia`), `test_aceite_achado35.py` + `test_e2e_browser_conciliacao_ui.py`; B2 (ACHADO-36) **parcial** (só módulo financeiro/provisões, conforme o item): 36 `showToast(..., true)` viraram `avisoPopup`, 164 restam no resto do sistema (higiene), `test_aceite_achado36.py`; Parte A (tela de Provisões unificada) **não entrou** — frente própria, decisão do Marcelo |
