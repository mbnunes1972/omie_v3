...
| 32 | guarda do F2-3 entrou no servidor (409), a tela da Conciliação Final continuou oferecendo Efetivar/Resolver em toda linha | `test_aceite_conciliacao_ui_item1.py` (flag `exige_veredito` derivada de `_PROV_FORA_DO_VEREDITO`, controle negativo movendo um código pra dentro da constante) + `test_e2e_browser_conciliacao_ui.py` (navegador — os três estados de linha batidos contra o JSON real do endpoint, tooltips do item 3, selo/toast do item 2) + `test_e2e_browser_ciclo_overlay.py` (navegador — item 4, Ciclo aberto esconde a negociação por baixo, `.modal-overlay` continua visível) | **CONSERTADO** |
