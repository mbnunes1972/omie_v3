...
| 45 | `budget_total` (venda) e `order_total` (CFO) vêm do mesmo XML e ninguém os compara — ambiente pode ser importado/orçado/contratado com margem zero ou negativa sem aviso | medido antes de escolher o momento: 0/12 `pool_ambientes` e 0/12 `arquivo_pe` reais (Homologação, único ambiente com dados) violavam a regra hoje; Integração/Produção zeradas — achado ao implementar: o pool JÁ TINHA trava (`avaliar_qualidade_xml`/`qa_selo='bloqueado'`, mais branda — quarentena, não recusa); `test_achado45_venda_maior_que_cfo.py` (6 — função pura incl. fronteira venda==CFO, HTTP no PE, tripwire do bypass de xml_compl, prova de que o pool fica na quarentena antiga); controle negativo confirmado | **PARCIALMENTE CONSERTADO** 02/09 — `venda_maior_que_cfo` (integracoes/promob_grupos.py) recusa NO UPLOAD só no PE (`ArquivoPE`, sem gate equivalente antes); o pool (`PoolAmbiente`) NÃO ganhou hard-reject — duplicaria a quarentena já testada, 2ª porta pro mesmo destino, não decidido sem perguntar (regra de 02/09); pergunta em aberto pro Marcelo: hard-reject também no pool, manter a quarentena, ou outra coisa |
